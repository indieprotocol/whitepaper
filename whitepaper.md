...
### Referral Program
Indie Protocol has an integrated one-level referral system. Basically, everyone interacting on Indie Protocol needs to deduct a small transaction fee. From that fee (currently) 20% go into the Working Budget (for future funding of development etc.) and the other 80% go into the referral program from where, the registrar (who paid the registration fee and assisted the registration process) as well as the referrer (who brought the user to the registrar) receive a reward. When an account is upgraded to a so called Verified Creator, it replaces registrar and referrer for the original user to receive a 80% refund on it's fees.
