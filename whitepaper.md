...
#### Statistics
The most important action that is mandatory to be an active distributor is to submit signed statistics to Indie Protocol. Those statistics need to be submitted for each category that the distributor is using from the protocol. They will be network dependent, as different types of digital media content have different metrics, but we can assume that at least they will include a top content list and a top collection list, and both include the usage and interactions details (like plays, likes or pages turned, etc) for each content or collection.

#### Publish Collections
Distributors are welcome to publish their curated collections in Indie Protocol. By doing this, they will not generate rewards from their usage in their own platform, but will generate rewards by the usage they collect from other distribution platforms. For some use cases this could be an interesting marketing strategy, to acquire new users or expose creators content to larger audiences, besides generating some cashflow back to the distributor.
...
Being an active member of the committee comes at a cost for executing all administrative and managerial tasks. Indie Protocol acknowledges this fact by rewarding committee members in core IND tokens per schedule cycle (block producers cycle). Depending on the valuation of IND, the committee can modify the amount of IND rewarded per cycle. As a start, committee members are rewarded with 1 IND per cycle. Those IND are taken from the working budget.

Should the committee member be required to produce extra work for the blockchain, besides regular approvals of accounts or parameter changes, then they are encouraged to submit a worker proposal to the network to be approved and paid by the working budget as any worker.

### Block Production
Block production in Indie Protocol is arranged through DPoS, which requires block producers to be voted into the active block producer rank, so that they start producing blocks and get block rewards. Given the governance system and quick re-tallying of votes, a misbehaving block producer can be dismissed within hours. Next to the actual selection of block producers, the voters also have a say over how many block producers should exist. As part of being a block producer comes the responsibility of deciding which protocol version they support and run, and this is probably the most important governance factor in the protocol, as anyone can propose a new version of the protocol and the changes need to be approved by significant portion of the block producers to activate.
